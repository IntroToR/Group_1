...
```
__Sulfur Concentration Dataset__test
...
write.table(overall_means, "volcano_dataset_overall_means.txt")
...
write.table(volcano_means_table, "volcano_dataset_means_by_primary_volcano_type.txt")
```
...
elevation_anova<-aov(elevation~primary_volcano_type, data=volcano)
elevation_anova
```
...
```{r}
earth <- "https://upload.wikimedia.org/wikipedia/commons/thumb/c/cf/A_large_blank_world_map_with_oceans_marked_in_blue.PNG/1280px-A_large_blank_world_map_with_oceans_marked_in_blue.PNG"
globejs(img = earth, lat=eruptions$latitude, long=eruptions$longitude, value = eruptions$vei)
```