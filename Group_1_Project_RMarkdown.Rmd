...
First step is to create scatter plots to visualize a possible linear relationship between the independent and dependent variable: "year" vs. sulfur concentration for each location and sulfur levels at NEEM vs. WDC. 

From the scatterplot it does not appear that there is a linear relationship between "year" and sulfur concentration for each location and NEEM vs. WDC
...
```{r}
#density plot for NEEM
dp_NEEM<- ggplot(sulfur, aes(x=neem))+
              geom_density(color="yellow", fill="yellow")+
              xlab("NEEM Sulfur Concentration (ng/g)") +
              ggtitle("Density of Sulfur Concentrations from NEEM")

#density plot for WDC
dp_WDC<- ggplot(sulfur, aes(x=wdc))+
              geom_density(color="orange", fill="orange")+
              xlab("WDC Sulfur Concentrations (ng/g)") +
              ggtitle("Density of Sulfur Concentrations from WDC")
  
```

After creating and checking the previous 3 plots, we have to test for correlation between year vs. sulfur concentrations in both locations and NEEM vs. WDC .Even though the disibutions of sulfur levels appears non-normal for NEEM and WDC, we will continue to test for correlation using the Pearson's "r" for the sake of the project. Our null hypothesis is that there is no correlation between year vs sulfur levels for NEEM and WDC, and NEEM vs WDC. The alternative hypothesis is there is a correlation between year vs. sulfur levels for NEEM and WDC, and NEEM vs. WDC.

All 3 tests revealed p-levels below 0.05. We reject the null hypothesis and conclude that there is a correlation between year and sulfur levels for NEEM and WDC
...
#calculating the Pearson's "r" for NEEM vs. WDC
cor(sulfur$neem, sulfur$wdc, method = "pearson", use = "complete.obs")
# testing if the correlation is statistically significant from 0
cor.test(sulfur$neem, sulfur$wdc, method="pearson")

```

Building a Linear Model - Building a linear model allows us to establish a relationship between year and sulfur levels at NEEM and WDC locations in the form ofsulfur levels as a function for year. Linear models for sulfur levels at both locations are shown below:

NEEM sulfur levels (ng/g) = 19.886023 - 0.005519 * year
WDC sulfur levels (ng/g) = 21.02073 - 0.01072 * year
WDC sulfur levels (ng/g) = 13.35411 + 0.06182 * neem (ng/g)
...
Checking if our linear model is statistically significant - The null hypothesis is that the coefficients associated with year and sulfur levels at NEEM/WDC, and NEEM vs WDC is equal to zero. The alternative hypothesis is that the coefficients are not equal to zero and there is a relationship between sulfur levels at NEEM/WDC and year, and NEEM vs. WDC.Our conclusions are noted below:
...
p-value for the linear model for NEEM vs. WDC was below the 0.05 threshold, we reject the null hypothesis and conclude that there is a relation between sulfur levels at NEEM and WDC
...
Here is just a simple line graph plotting tree ring Z-scores and European Temperature Index historical data.
...
First step is to create scatter plots to visualize a possible linear relationship between the independent and dependent variable: "year" vs. "n_tree"/"europe_temp_index", and "n_tree" vs "europe_temp_index".

From the scatter plots it does not appear that there is a linear relationship between year vs. "n_tree"/"europe_temp_index". However, it seems that "n_tree" vs "europe_temp_index" follows a strong, positive linear relationship
```{r}
...
Second step is to create boxplots to check for outliers for the variables "n_Tree" and "europe_temp_index"; From both boxplots we conclude that there are a moderate amount of outliers
```{r}
#Boxplot for "n_tree"
...
Next step is create a density plot to see if "n_tree" and "europe_temp_index" follows a normal distribution. From the density plots, we see that "n_tree" and "europe_temp_index" follows a normal distribution.

```{r}
#density plot for "n_tree"
dp_ntree<- ggplot(tree_rings, aes(x=n_tree))+
              geom_density(color="green", fill="green")+
              xlab("Tree ring z-scores relative to year = 1000-1099") +
              ggtitle("Density of Tree ring z-scores")

#density plot for "europe_temp_index"
dp_ntree<- ggplot(tree_rings, aes(x=europe_temp_index))+
              geom_density(color="red", fill="red")+
              xlab("Pages 2K Temperature for Europe in Celsius") +
              ggtitle("Density of Pages 2K Temperature for Europe in Celsius")
```

After creating and checking the previous 3 plots, we have to test for correlation between year vs. "n_tree"/"europe_temp_index", and "n_tree" vs. "europe_temp_index".. Our null hypothesis is that there is no correlation between year vs. "n_tree"/"europe_temp_index", and "n_tree" vs. "europe_temp_index". The alternative hypothesis is there is a correlation between year vs. "n_tree"/"europe_temp_index", and "n_tree" vs. "europe_temp_index".

All 3 tests revealed p-levels below 0.05. We reject the null hypothesis and conclude that there is a correlation between year and "n_tree"/"europe_temp_index", and "n_tree" vs. "europe_temp_index"

```{r}
#calculating the Pearson's "r" for year vs. "n_tree"
cor(tree_rings$year, tree_rings$n_tree, method = "pearson", use = "complete.obs")
#testing if the correlation is statistically significant from 0
cor.test(tree_rings$year, tree_rings$n_tree, method="pearson")

#calculating the Pearson's "r" for year vs. "europe_temp_index"
cor(tree_rings$year, tree_rings$europe_temp_index, method = "pearson", use = "complete.obs")
#testing if the correlation is statistically significant from 0
cor.test(tree_rings$year, tree_rings$europe_temp_index, method="pearson")

#calculating the Pearson's "r" for "n_tree" vs. "europe_temp_index"
cor(tree_rings$n_tree, tree_rings$europe_temp_index, method = "pearson", use = "complete.obs")
# testing if the correlation is statistically significant from 0
cor.test(tree_rings$n_tree, tree_rings$europe_temp_index, method="pearson")
```

Building a Linear Model - Building a linear model allows us to establish a relationship between year and "n_tree"/"europe_temp_index", and "n_tree" vs. "europe_temp_index in the form of "n_tree"/"europe_temp_index" as a function of year and "europe_temp_index" as a function of "n_tree", respectively.

Linear models are shown below

Tree ring z-score = -0.3668420 - -0.0001842 * year
Pages 2K Temperature for Europe (C) = -0.0067512 - -0.0001582 * year
Pages 2K Temperature for Europe (C) = 0.1073 + 0.4941 * Tree ring z-score
...
Checking if our linear model is statistically significant - The null hypothesis is that the coefficients associated with year and "sulfur levels"n_tree"/"europe_temp_index, and "n_tree" vs. "europe_temp_index" is equal to zero. The alternative hypothesis is that the coefficients are not equal to zero and there is a relationship between year and "sulfur levels"n_tree"/"europe_temp_index, and "n_tree" vs. "europe_temp_index".
...
The appropriate p-values for all three linear models was below the 0.05 threshold. We reject the null hypothesis and conclude that the coefficients are not equal to zero and there is a relationship between year and "sulfur levels"n_tree"/"europe_temp_index, and "n_tree" vs. "europe_temp_index".

Savings the means of "n_tree" and "europe_temp_index" as a text file:
```{r}
write.table(mean(tree_rings$n_tree, na.rm = TRUE), "nTree_mean.txt", sep = "\t")
write.table(mean(tree_rings$europe_temp_index, na.rm = TRUE), "ETI_mean.txt", sep = "\t")
```
...
eruptions2 <- filter(eruptions2, evidence_method_dating != "Historical Observations")
