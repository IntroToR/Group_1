...
__NOTE:__ In this finalized RMarkdown document, we have eliminated the functions necessary for saving individual tables or figures. These were initially done using a file path on a local machine, and were causing issues for other group members when trying to knit the final document together into HTML format. Once all graphs, tables, and data sets were saved into their final form and uploaded to respective folders on Github, this code was removed. To see relevant code for saving files using the `write.table()` and `ggsave()` functions, please view the file titled "Group_1_Project_RMarkdown_With_Filepath_Saving_Code.Rmd".
