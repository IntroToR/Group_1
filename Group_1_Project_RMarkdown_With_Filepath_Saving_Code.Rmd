...
write.table(sulfur_long, file.path(data_sets_path, "sulfur_dataset_pivot_long.txt"))
...
write.table(tree_rings_long, file.path(data_sets_path, "tree_rings_dataset_pivot_long.txt"))
...
This is a histogram showing the number of volcanos that have a known eruption within a certian number of years in the past. Most volcano eruptions that we are aware of fall most recently, which tracks well with the fact established above that most known volcanic eruptions fall under the "Eruption Observed" category.
```{r}
...
ggplot(volcano, aes(x=reorder(primary_volcano_type, elevation, na.rm=T), y=elevation))+
  ...
  labs(y="Elevation (Meters above Sea Level)", x="Volcano Type")+
  theme(panel.grid=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.95))+
...
ggplot(eruptions, aes(x=vei))+
  ...
  theme(panel.grid = element_blank())+
  ggsave(file.path(figure_path, "vei_histogram.pdf"))
```

___VEI Visualized over Longitude and Latitude___

These two count graphs provide a visualization of the number of volcanoes, and their respective VEI, plotted over Longitude and Latitude. this is achieved by simply plotting the VEI against the data points for Latitude and Longitude respectively. The size of each point represents the number of volcanoes of a certain VEI at a certain Latitude or Longitude. Using these plots we can see the distribution of intense volcanoes between the Eastern and Western hemispheres, as well as the Northern and Southern hemispheres.
```{r}
...
ggplot(eruptions, aes(x=latitude, y=vei))+
  geom_count(na.rm = T, col = "orange")+
  ggtitle("Volcano Explosivity Index Distributed Over Latitude")+
  labs(x="Latitude", y="VEI")+
  ggsave(file.path(figure_path, "vei_by_latitude.pdf"))

ggplot(eruptions, aes(x=longitude, y=vei))+
  geom_count(na.rm = T, col = "blue")+
  ggtitle("Volcano Explosivity Index Distributed Over Longitude")+
  labs(x="Longitude", y="VEI")+
  ggsave(file.path(figure_path, "vei_by_longitude.pdf"))
```
...
write.table(eruptions_updated, file.path(data_sets_path,"updated_eruptions_data_set.txt"))
...
ggplot(eruptions_updated, aes(x=vei, y=eruption_length_in_days))+
  ...
  theme(panel.grid = element_blank())+
  ggsave(file.path(figure_path, "eruption_length_by_vei.pdf"))
...
ggplot(eruptions_updated, aes(x=vei, y=eruption_length_in_days))+
  ...
  labs(x="VEI", y="Eruption Lenght (Days)")+
  theme(panel.grid = element_blank())+
  ggsave(file.path(figure_path, "eruption_length_by_vei_in_one_SD.pdf"))
```
...
ggplot(eruptions, aes(x=evidence_method_dating))+
  ...
  ggsave(file.path(figure_path, "eruptions_by_evidence_method.pdf"))
...
ggplot(eruptions2, aes(x=evidence_method_dating))+
  ...
  ggsave(file.path(figure_path, "eruptions_by_evidence_method_without_hist_obs.pdf"))
```
